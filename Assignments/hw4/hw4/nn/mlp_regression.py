...
class MLPRegression(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, num_hidden_units=10, step_size=.005, init_param_scale=0.01, max_num_epochs = 5000):
        ...
        self.b1 = nodes.ValueNode(node_name=f"b1") # to hold the bias parameter (scalar)

        self.w2 = nodes.ValueNode(node_name="w2") # to hold the parameter vector
        self.b2 = nodes.ValueNode(node_name=f"b2")
       
   
        self.prediction = nodes.AffineNode(x = self.x, w = self.w1, b = self.b1,node_name="hidden_layer")
        self.prediction = nodes.TanhNode(a = self.prediction, node_name=f"activation")

        self.prediction = nodes.VectorScalarAffineNode(x = self.prediction , w=self.w2,b = self.b2, node_name="output_layer")
